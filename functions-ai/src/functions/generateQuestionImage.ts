...
export const generateQuestionImage = onCall(
  ...
  async (request): Promise<GenerateImageResponse> => {
    ...
    // Validate path parameters - either presentation or quiz path
    const isPresentationPath = presentationId && slideId;
    const isQuizPath = (quizId || tempId) && questionIndex !== undefined;

    if (!isPresentationPath && !isQuizPath) {
      throw new HttpsError(
        'invalid-argument',
        'Either (presentationId + slideId) or (quizId/tempId + questionIndex) is required'
      );
    }

    if (isQuizPath && typeof questionIndex !== 'number') {
      throw new HttpsError(
        'invalid-argument',
        'Question index must be a number'
      );
    }
    ...
    try {
      ...
      // Determine storage path based on context (presentation or quiz)
      let filePath: string;
      if (presentationId && slideId) {
        // Presentation slide image
        filePath = `presentations/${presentationId}/slides/${slideId}/image.png`;
      } else if (quizId) {
        // Saved quiz image
        filePath = `quizzes/${quizId}/questions/${questionIndex}/image.png`;
      } else {
        // Temp quiz image
        filePath = `temp/${tempId}/questions/${questionIndex}/image.png`;
      }
      ...
      // Add TTL metadata for temp quiz images (24 hours from now)
      // Note: presentation images are permanent (tied to saved presentation)
      const isTemp = !quizId && !presentationId;
      ...
      const context = presentationId ? 'presentation' : 'quiz';
      console.log(
        `Image generated for ${context} by user ${request.auth.uid}: ${filePath}`
      );
