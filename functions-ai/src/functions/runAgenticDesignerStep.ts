...
function buildContextString(
  ...
  // For step 11, note that the Solution Map infographic was generated in step 10
  if (stepNumber === 11 && imageUrls?.[10]) {
    contextString += '\nNote: The AI Data Foundation Map infographic was successfully generated in Step 10. Reference it in the report as "See the AI Data Foundation Map infographic generated in Step 10."\n';
  }
...
4. Output ONLY the markdown content for the design document. No conversational filler.`;
      ...
      if (data.stepNumber === 10) {
        try {
          ...
          console.error('Infographic image generation failed, continuing with text output:', imageError);
        }
      }
      ...
      if (imageUrl) {
        updateData[`imageUrls.${data.stepNumber}`] = imageUrl;
      }
