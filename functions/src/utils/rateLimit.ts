 ...
 *
 * Tradeoffs:
 * - Accurate across all function instances
 * - Adds ~50-100ms latency per check (Firestore transaction)
 * - Use a TTL policy on the `rateLimits` collection to auto-delete expired entries
 */
export async function enforceRateLimitFirestore(
  ...
  // Sanitize identifier for Firestore document ID (no slashes)
