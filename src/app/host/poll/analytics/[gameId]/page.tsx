...
import { usePollAnalytics } from './hooks/use-poll-analytics';
...
export default function PollAnalyticsPage() {
  ...
  const { game, poll, analytics, loading, analyticsExists } = usePollAnalytics(gameId);
  ...
  useEffect(() => {
    ...
    // Verify this is a poll game
    if (game.activityType !== 'poll') return;
  ...
  const generateAnalytics = async () => {
    ...
    try {
      const computePollAnalytics = httpsCallable(functions, 'computePollAnalytics');
      await computePollAnalytics({ gameId });
      // Analytics will be loaded via the real-time listener in usePollAnalytics
    } catch (error: any) {
      console.error('[PollAnalytics] Error generating analytics:', error);
  ...
  if (!game) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8">
        ...
        <p className="text-muted-foreground mb-6">This poll session may have been deleted.</p>
        <Button asChild>
  ...
  if (game.hostId !== user.uid) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8">
        ...
        <p className="text-muted-foreground mb-6">You can only view analytics for your own polls.</p>
        <Button asChild>
          <Link href="/host">Back to Dashboard</Link>
        </Button>
      </div>
  ...
  if (game.activityType !== 'poll') {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8">
        <AlertTriangle className="h-16 w-16 text-destructive mb-4" />
        <h1 className="text-2xl font-bold mb-2">Not a Poll</h1>
        <p className="text-muted-foreground mb-6">This page is only for poll analytics.</p>
        <Button asChild>
          <Link href="/host">Back to Dashboard</Link>
        </Button>
      </div>
  ...
  if (game.state !== 'ended') {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8">
        ...
        <h1 className="text-2xl font-bold mb-2">Poll In Progress</h1>
        <p className="text-muted-foreground mb-6">Analytics are only available after the poll ends.</p>
        <Button asChild>
          <Link href={`/host/poll/game/${gameId}`}>Go to Poll</Link>
        </Button>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-8">
        ...
        <p className="text-muted-foreground mb-6">
          Processing your poll results... This may take a moment.
        </p>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-xl font-bold">{analytics.pollTitle}</h1>
              </div>
              <p className="text-sm text-muted-foreground">
                {analytics.totalParticipants} participant{analytics.totalParticipants !== 1 ? 's' : ''} &bull; {analytics.totalQuestions} question{analytics.totalQuestions !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
        </div>
      ...
      <main className="container mx-auto px-4 py-6">
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="questions">
            <QuestionsTab analytics={analytics} poll={poll} />
          </TabsContent>
        </Tabs>
      </main>
    </div>
