...
interface FinalLeaderboardViewProps {
  ...
  totalPlayers: number;
  /** Maximum number of players to display (default: 20) */
  maxDisplay?: number;
}

/**
 * Displays the final leaderboard with podium-style top 3 and remaining players.
 * Uses pre-sorted data from the server-side aggregate document.
 *
 * Reused by:
 * - Standalone quiz game final screen
 * - Presentation leaderboard slide (final mode)
 */
export function FinalLeaderboardView({
  ...
  const remainingPlayers = topPlayers.slice(3, maxDisplay);

  return (
    <div className="w-full max-w-3xl">
      {/* Total players indicator */}
      {totalPlayers > maxDisplay && (
        <p className="text-muted-foreground text-center mb-4">
          Showing top {maxDisplay} of {totalPlayers} players
        </p>
      )}
      ...
      {/* Remaining Players (4-maxDisplay) */}
