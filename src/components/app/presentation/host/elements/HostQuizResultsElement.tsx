...
interface HostQuizResultsElementProps {
  element: SlideElement;
  slides: PresentationSlide[];
  gameId: string;
}

export function HostQuizResultsElement({ element, slides, gameId }: HostQuizResultsElementProps) {
  const { getElementResponses } = useResponses(gameId);
  const [distribution, setDistribution] = useState<number[]>([]);

  // Find source quiz element
  ...
  const config = sourceElement?.quizConfig;
  ...
  useEffect(() => {
    ...
    const loadResponses = async () => {
      ...
      const dist = new Array(config.answers.length).fill(0);
      responses.forEach((r) => {
        if (r.answerIndex !== undefined && r.answerIndex < dist.length) {
          dist[r.answerIndex]++;
        }
      });
        ...
        {config.answers.map((answer, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-3">
              <span className={`text-sm font-medium w-28 text-right truncate ${isCorrect ? 'text-green-600 font-bold' : ''}`}>
